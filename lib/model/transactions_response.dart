...
class Tx {
  ...
  String? getAmount() {
    if (token_contract_address_hash != null) {
      return amount;
    }
    return value;
  }
  ...
  TokenSymbols getSymbol() {
    if (token_contract_address_hash != null) {
      return Web3Utils.getSymbolFromAddress(token_contract_address_hash!.hex);
    }
    return Web3Utils.getNativeToken();
  }

  DateTime getInsertedAt() {
    if (token_contract_address_hash == null) {
      return block?.timestamp ?? DateTime.now();
    }
    return insertedAt!;
  }
}
